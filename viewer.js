...
let chosen='gris', inAR=false;
const factors={gris:[.36,.38,.37,1],arena:[.64,.54,.40,1],petroleo:[.025,.21,.24,1]};
...
function applyMaterial(){const material=viewer.model?.materials.find(m=>m.name==='Tapizado');if(material)material.pbrMetallicRoughness.setBaseColorFactor(factors[chosen]);}
...
viewer.addEventListener('ar-status',event=>{
  ...
  if(state==='not-presenting'&&!viewer.src.includes('sofa-190-'+chosen+'.glb'))selectColor(chosen);
...
for(const id of fitFields)document.querySelector('#fit-'+id).addEventListener('input',()=>{
 ...
 const required=[189.99,104.13,89.19],labels=['ancho','fondo','altura'];
